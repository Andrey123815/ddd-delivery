...
# Сервис Geo (docker-compose)

После запуска `docker-compose` сервис Geo слушает **тот же порт**, что и в примере доступа из браузера: `http://localhost:5004` (или `0.0.0.0:5004` внутри контейнера). Приложение доставки подключается к Geo **по gRPC**, не по HTTP.

В `.env` задайте адрес в формате **`host:port` без схемы `http://`**:

```
GEO_SERVICE_GRPC_HOST=localhost:5004
```

Если указать URL вида `http://localhost:5004`, клиент может получить ответ не в формате gRPC (например, `text/plain` и HTTP 500). В коде префикс `http://` / `https://` при необходимости отбрасывается, но корректное значение — всё равно `localhost:5004`.
