...
func Test_ReserveCourierForOrderReturnsErrorWhenOrderNil(t *testing.T) {
	...
	c, _ := courierModel.NewCourier("Курьер", 5, loc)
...
func Test_ReserveCourierForOrderReturnsClosestCourierWhenSeveralCanTake(t *testing.T) {
	...
	// Courier near: speed 10 -> time = 1/10 = 0.1
	cNear, _ := courierModel.NewCourier("Ближний", 10, locNear)
	_ = cNear.AddStoragePlace("Склад", 100)
	// Courier far: speed 1 -> time = 18/1 = 18
	cFar, _ := courierModel.NewCourier("Дальний", 1, locFar)
	_ = cFar.AddStoragePlace("Склад", 100)

	svc := NewOrderDispatcher()

	// Near first in list -> should pick near (closer)
	...
	cFar2, _ := courierModel.NewCourier("Дальний2", 1, locFar)
	_ = cFar2.AddStoragePlace("Склад", 100)
	cNear2, _ := courierModel.NewCourier("Ближний2", 10, locNear)
	_ = cNear2.AddStoragePlace("Склад", 100)
...
func Test_ReserveCourierForOrderSkipsCouriersThatCannotTake(t *testing.T) {
	loc, _ := kernel.NewLocation(1, 1)
	ord, _ := orderModel.NewOrder(loc, 10)
	cNoStorage, _ := courierModel.NewCourier("Без склада", 5, loc)
	cWithStorage, _ := courierModel.NewCourier("Со складом", 5, loc)
	_ = cWithStorage.AddStoragePlace("Склад", 100)
	svc := NewOrderDispatcher()
