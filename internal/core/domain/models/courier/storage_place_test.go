...
func Test_NewStoragePlaceValidAtValidParams(t *testing.T) {
	place, err := NewStoragePlace("Склад A", mustNewVolume(100))
	if err != nil {
	...
	if place.TotalVolume().Value() != 100 {
		t.Errorf("TotalVolume() = %d, want 100", place.TotalVolume().Value())
	}
...
func Test_NewStoragePlaceCreateErrorAtInvalidName(t *testing.T) {
	_, err := NewStoragePlace("", mustNewVolume(100))
	if err == nil {
...
func Test_NewStoragePlaceCreateErrorAtInvalidVolume(t *testing.T) {
	_, err := NewStoragePlace("Склад", mustNewVolume(0))
	if err == nil {
		t.Error("expected error for volume 0")
	}

	_, err = NewStoragePlace("Склад", mustNewVolume(-5))
	if err == nil {
...
func Test_StoragePlaceEqualsReturnsCorrectResult(t *testing.T) {
	a, _ := NewStoragePlace("A", mustNewVolume(10))
	if !a.Equals(a) {
		t.Error("place should equal itself")
	}

	b, _ := NewStoragePlace("B", mustNewVolume(10))
	if a.Equals(b) {
...
func Test_StoragePlaceCanStoreReturnsErrorAtInvalidVolume(t *testing.T) {
	place, _ := NewStoragePlace("Склад", mustNewVolume(50))

	_, err := place.CanStore(mustNewVolume(0))
	if err == nil {
		t.Error("expected error for volume 0")
	}

	_, err = place.CanStore(mustNewVolume(-1))
	if err == nil {
		t.Error("expected error for negative volume")
	}
}
...
func Test_StoragePlaceCanStoreReturnsTrueWhenFreeAndEnoughVolume(t *testing.T) {
	...
	ok, err := place.CanStore(mustNewVolume(30))
	if err != nil {
	...
	ok, _ = place.CanStore(mustNewVolume(50))
	if !ok {
		t.Error("free place with exact volume should allow store")
	}
	
	ok, _ = place.CanStore(mustNewVolume(51))
	if ok {
...
func Test_StoragePlaceCanStoreReturnsFalseWhenOccupied(t *testing.T) {
	...
	if err := place.Store(orderID, mustNewVolume(20)); err != nil {
		t.Fatalf("Store failed: %v", err)
	}

	ok, err := place.CanStore(mustNewVolume(10))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func Test_StoragePlaceStoreSucceedsAtValidParams(t *testing.T) {
	...
	err := place.Store(orderID, mustNewVolume(30))
	if err != nil {
		t.Fatalf("Store failed: %v", err)
	}
...
func Test_StoragePlaceStoreReturnsErrorAtInvalidVolume(t *testing.T) {
	...
	err := place.Store(uuid.New(), mustNewVolume(0))
	if err == nil {
		t.Error("expected error for volume 0")
	}
}
...
func Test_StoragePlaceStoreReturnsErrorWhenNoCapacity(t *testing.T) {
	...
	err := place.Store(uuid.New(), mustNewVolume(60))
	if err == nil {
...
func Test_StoragePlaceStoreReturnsErrorWhenAlreadyOccupied(t *testing.T) {
	place, _ := NewStoragePlace("Склад", mustNewVolume(50))
	_ = place.Store(uuid.New(), mustNewVolume(20))

	err := place.Store(uuid.New(), mustNewVolume(10))
	if err == nil {
...
func Test_StoragePlaceClearSucceedsWhenOccupiedWithMatchingOrderId(t *testing.T) {
	place, _ := NewStoragePlace("Склад", mustNewVolume(50))
	orderID := uuid.New()
	_ = place.Store(orderID, mustNewVolume(20))
